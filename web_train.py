...
# --- CLASE DE TRANSFORMACIÓN PERSONALIZADA PARA DETECCIÓN DE BORDES / UMBRAL ADAPTATIVO ---
class EdgeDetectionTransform:
    def __init__(self, method='canny', low_threshold=50, high_threshold=150,
                 adaptive_block_size=11, adaptive_c=2): # Añadimos parámetros para umbral adaptativo
        self.method = method
    ...
    def __call__(self, img):
        ...
        elif self.method == 'adaptive_threshold': # Nuevo método para umbral adaptativo
            # Aplicar umbral adaptativo
            # cv2.ADAPTIVE_THRESH_GAUSSIAN_C o cv2.ADAPTIVE_THRESH_MEAN_C
            # cv2.THRESH_BINARY o cv2.THRESH_BINARY_INV
            edge_img = cv2.adaptiveThreshold(gray, 255,
                                             cv2.ADAPTIVE_THRESH_GAUSSIAN_C, # O cv2.ADAPTIVE_THRESH_MEAN_C
                                             cv2.THRESH_BINARY,             # O cv2.THRESH_BINARY_INV
                                             self.adaptive_block_size,      # Debe ser un número impar (e.g., 3, 5, 7, ...)
                                             self.adaptive_c)               # Constante restada de la media o suma ponderada
        else: #
            raise ValueError(f"Método de detección de bordes '{self.method}' no soportado. Usa 'canny', 'sobel', 'laplacian' o 'adaptive_threshold'.") #
...
class RandomEdgeDetection(object): #
    ...
    def __init__(self, method='canny', low_threshold=50, high_threshold=150, p=0.5,
                 adaptive_block_size=11, adaptive_c=2): # Añadimos parámetros
        self.edge_transform = EdgeDetectionTransform(method=method, low_threshold=low_threshold, high_threshold=high_threshold,
                                                     adaptive_block_size=adaptive_block_size, adaptive_c=adaptive_c) # Pasamos los parámetros
        self.p = p #
...
train_transforms = transforms.Compose([ #
    ...
    # **CAMBIO AQUI:** Ahora puedes usar 'adaptive_threshold'
    RandomEdgeDetection(method='adaptive_threshold', p=0.3, # Aplica Umbral Adaptativo en el 30% de las imágenes de entrenamiento
                        adaptive_block_size=11, adaptive_c=2), # Ajusta estos valores para tu dataset
    transforms.ToTensor(), #
...
val_transforms = transforms.Compose([ #
    ...
    # Si quieres evaluar el modelo con imágenes de bordes/umbrales adaptativos en validación (consistentemente):
    # EdgeDetectionTransform(method='adaptive_threshold', adaptive_block_size=11, adaptive_c=2),
    transforms.ToTensor(), #
    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]) #
])
...
model_save_path = 'modelo_micorrizas_3clases_pytorch3.pth' #
